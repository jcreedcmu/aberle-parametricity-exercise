...
module _ (A : Set) where
 ...
 displayed : Set₁
 displayed = Σ Set (λ B → B → A)

 fore : indexed → displayed
 fore P = Σ A P , fst

 back : displayed → indexed
 back (B , π) a = fiber π a

 sect : (x : displayed) → fore (back x) ≡ x
 sect (B , π) = {!≡-×  ? ?!}

 retr : section back fore
 retr x = {!!}

 lemma : indexed ≡ displayed
 lemma = isoToPath (iso fore back sect retr)
